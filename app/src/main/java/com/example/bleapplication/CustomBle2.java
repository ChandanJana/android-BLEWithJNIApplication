...
public class CustomBle2 {
    ...
    private final String TAGG = CustomBle2.class.getSimpleName();
    ...
    private static CustomBle2 instance = null;
    ...
    private CustomBle2(Context context) {
    ...
    public static CustomBle2 getInstance(Context context) {
        //synchronized (instance) {
        if (instance == null)
            return new CustomBle2(context);
        else
            return instance;
        //}
    ...
    public Map<String, BluetoothDevice> getScanResultsInMap() {
        return scanResults;
    }
    ...
    public boolean matchAdvertisingData(String dataString){

        boolean isMatched = false;

        for ( ScanResult item: listScanResults) {
            if (isMatched)
                break;
            int index = 0;
            while (index < item.getScanRecord().getBytes().length) {
                byte[] bytesArr = item.getScanRecord().getBytes();
                int length = Integer.getInteger(String.valueOf(bytesArr[index++]));
                //Done once we run out of records
                if (length == 0)
                    break;
                int  type = Integer.getInteger(String.valueOf(bytesArr[index]));
                //Done if our record isn't a valid type
                if (type == 0)
                    break;
                byte[] dataByte = Arrays.copyOfRange(bytesArr, index + 1, index + length);
    ...
    public boolean matchAdvertisingData(byte[] dateBytes){
        ...
        for ( ScanResult item: listScanResults) {
            ...
            while (index < item.getScanRecord().getBytes().length) {
                ...
                isMatched = Arrays.equals(dataByte, dateBytes);
